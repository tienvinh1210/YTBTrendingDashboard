...
# --- Paths: this file lives at <repo>/ml/stage_1_xg_boost.py (repo = YTBTrendingDashboard) ---
_ML = Path(__file__).resolve().parent
_DASH = _ML.parent
MODELS_DIR = _DASH / "models"
...
def _default_stage1_training_csv() -> Path:
    override = os.environ.get("STAGE1_TRAINING_CSV")
    if override:
        return Path(override)
    for candidate in (
        _ML / "data" / "stage1_training_data.csv",
        _DASH.parent / "stage1_training_data.csv",
    ):
        if candidate.is_file():
            return candidate
    return _ML / "data" / "stage1_training_data.csv"
...
def train_and_save(
    ...
    csv_path = Path(csv_path) if csv_path is not None else _default_stage1_training_csv()
