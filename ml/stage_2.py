...
def _default_stage2_training_csv() -> Path:
    override = os.environ.get("STAGE2_TRAINING_CSV")
    if override:
        return Path(override)
    for candidate in (
        _ML / "data" / "stage2_training_data.csv",
        _DASH.parent / "stage2_training_data.csv",
    ):
        if candidate.is_file():
            return candidate
    return _ML / "data" / "stage2_training_data.csv"
...
def train_stage2_and_save(
    ...
    csv_path = Path(csv_path) if csv_path is not None else _default_stage2_training_csv()
